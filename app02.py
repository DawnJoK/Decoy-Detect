...
import numpy as np
import joblib
from metadata_utils import fetch_app_metadata # Assuming this is a separate utility file
...
MYSQL_PASSWORD = '302005' # Your MySQL password
MYSQL_DB = 'decoy_detect_db' # Your MySQL database name
...
try:
    ...
    print(f"✅ MySQL connected successfully to database '{MYSQL_DB}'!")
except Exception as e:
    print(f"❌ Error connecting to MySQL: {e}")
    mysql_conn = None # Ensure mysql_conn is None if connection fails
...
try:
    ...
    # exit() # Do not exit, allow the app to run even if model loading fails for other routes
...
def predict():
    ...
    try:
        ...
        return jsonify({
            ...
            "url_for_comments": url # Pass the URL back for comment handling
        })
...
def add_comment():
    ...
    try:
        with mysql_conn.cursor() as cursor:
            sql = "INSERT INTO comments (url, comment) VALUES (%s, %s)"
            cursor.execute(sql, (url, comment))
        mysql_conn.commit() # Commit changes to the database
        return jsonify({"message": "Comment added successfully"}), 201
    except Exception as e:
        print(f"Error adding comment to MySQL: {e}")
...
def get_comments():
    ...
    try:
        with mysql_conn.cursor() as cursor:
            sql = "SELECT comment, timestamp FROM comments WHERE url = %s ORDER BY timestamp DESC"
            cursor.execute(sql, (url,))
            comments = cursor.fetchall() # Fetch all results as dictionaries

        # Convert datetime objects to string for JSON serialization
        # Ensure 'timestamp' is present before attempting to format
        for c in comments:
            if 'timestamp' in c and isinstance(c['timestamp'], datetime):
                c['timestamp'] = c['timestamp'].isoformat()
            else:
                c['timestamp'] = None # Or handle cases where timestamp might be missing or not datetime

        return jsonify({"comments": comments}), 200
    except Exception as e:
        print(f"Error fetching comments from MySQL: {e}")
